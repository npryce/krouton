...
class Repetition {
    ...
    fun route_repeating_single_string() {
        assertThat((+string).parse("/x/y/z"), present(equalTo(listOf("x", "y", "z"))))
        assertThat(("foo" / +string).parse("/foo/a/b/c/d"), present(equalTo(listOf("a", "b", "c", "d"))))
    }

    @Test
    fun route_repeating_parsed_element() {
        assertThat(("bar"/+int).parse("/bar/1/2/3/4"), present(equalTo(listOf(1,2,3,4))))
    }

    @Test
    fun route_repeating_elements_in_middle_of_path() {
        assertThat(("x"/+int/string).parse("/x/1/2/3/y"), present(equalTo(Pair(listOf(1,2,3),"y"))))
    ...
    fun route_repeating_composite_path_elements() {
        ...
        assertThat(route.parse("/a/1/b/2/c/3"), present(equalTo(listOf("a" to 1, "b" to 2, "c" to 3))))
    }

    @Test
    fun route_empty_list() {
        assertThat((+string).parse("/"), present(equalTo(emptyList())))
    }
